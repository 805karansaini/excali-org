...
export function EnhancedAutoHidePanel({ onNewCanvas, onCanvasSelect }: Props) {
  ...
  const handleTogglePanel = useCallback(() => {
    ...
    eventBus.emit(InternalEventTypes.PANEL_VISIBILITY_CHANGED, { isVisible: newVisible });
  ...
  const handleNewCanvasEnhanced = useCallback(async () => {
    try {
      ...
      // Show error to user
      dispatch({ type: 'SET_ERROR', payload: 'Failed to create canvas: ' + (error instanceof Error ? error.message : String(error)) });
  ...
  const handleMouseLeave = () => {
    if (!state.isPanelPinned && !isResizing) {
      timeoutRef.current = setTimeout(() => {
        ...
        eventBus.emit(InternalEventTypes.PANEL_VISIBILITY_CHANGED, { isVisible: false });
      }, 300);
    }
  };
  ...
  const handleCanvasSelect = async (canvas: UnifiedCanvas) => {
    try {
      ...
      dispatch({ type: 'SET_ERROR', payload: 'Failed to select canvas: ' + (error instanceof Error ? error.message : String(error)) });
    }
  };
